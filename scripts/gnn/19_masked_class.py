"""
19: Masked Flavor Class Prediction
Task: given recipe context (n-1 ingredients), predict the masked ingredient's flavor class.
Models:
  A - MLP on class vector context (baseline)
  B - linear probe on GNN embedding context (FlavorHGN / finetune_optimal)
  C - linear probe on FlavorClass-HGN embedding (18-D compat-weighted)
Metrics: Acc@1, Acc@3
"""
import ast, csv, json, random, sqlite3, os
import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
from torch_geometric.data import HeteroData
from torch_geometric.nn import HGTConv
from sklearn.metrics import accuracy_score

DB_PATH = "flavorgraph_v2.db"
DATA_DIR = "data"
SEEDS = [42, 123, 2024]
FLAVOR_KEYS = ['alcohol', 'ester', 'sulfur', 'aldehyde', 'nitrogen', 'acid', 'terpene', 'aromatic', 'phenol', 'other']
K = len(FLAVOR_KEYS)


def load_ingredients():
    conn = sqlite3.connect(DB_PATH)
    rows = conn.execute("SELECT id, name FROM ingredient ORDER BY id").fetchall()
    conn.close()
    name2idx = {r[1].lower(): i for i, r in enumerate(rows)}
    idx2name = {i: r[1] for i, r in enumerate(rows)}
    return name2idx, idx2name, len(rows)


def load_profiles(name2idx):
    raw = json.load(open(os.path.join(DATA_DIR, "ingredient_flavor_profile.json")))
    profiles = {}
    for name, d in raw.items():
        if name.lower() in name2idx:
            total = d.get('total', 1) or 1
            vec = np.array([d.get(k, 0) / total for k in FLAVOR_KEYS], dtype=np.float32)
            profiles[name2idx[name.lower()]] = vec
    return profiles  # idx -> vec


def dominant_class(vec):
    return int(np.argmax(vec))


def build_recipe_dataset(name2idx, profiles, min_matched=3):
    """Returns list of (context_idxs, target_idx, target_class)."""
    data = []
    with open(os.path.join(DATA_DIR, "RAW_recipes.csv")) as f:
        reader = csv.DictReader(f)
        for row in reader:
            try:
                ings = ast.literal_eval(row['ingredients'])
            except Exception:
                continue
            matched = [name2idx[g.lower()] for g in ings if g.lower() in name2idx]
            matched = [m for m in set(matched) if m in profiles]
            if len(matched) < min_matched:
                continue
            # one sample per recipe: pick one random target
            target = matched[random.randint(0, len(matched)-1)]
            context = [m for m in matched if m != target]
            cls = dominant_class(profiles[target])
            data.append((context, target, cls))
    return data


def build_gnn_model(n_ing, n_cmp, hidden=64, layers=3):
    class FlavorHGN(nn.Module):
        def __init__(self):
            super().__init__()
            self.ing_proj = nn.Linear(n_ing, hidden)
            self.cmp_proj = nn.Linear(n_cmp, hidden)
            metadata = (
                ['ingredient', 'compound'],
                [('ingredient','has','compound'),
                 ('compound','rev_has','ingredient'),
                 ('ingredient','pairs','ingredient')]
            )
            self.convs = nn.ModuleList(
                [HGTConv(hidden, hidden, metadata, heads=2) for _ in range(layers)]
            )
        def forward(self, data):
            x_dict = {
                'ingredient': self.ing_proj(data['ingredient'].x),
                'compound':   self.cmp_proj(data['compound'].x),
            }
            ei = {k: data[k].edge_index for k in data.edge_types}
            for conv in self.convs:
                x_dict = conv(x_dict, ei)
                x_dict = {k: F.relu(v) for k, v in x_dict.items()}
            return x_dict['ingredient']
    return FlavorHGN()


def load_graph_data(name2idx, n_ing, n_cmp, cmp_id2idx, ic_edges, pairs, phi=None, profiles_raw=None, config='A'):
    data = HeteroData()
    one_hot = torch.eye(n_ing)

    if config == 'C' and profiles_raw is not None:
        class_vecs = torch.zeros(n_ing, K)
        for name, idx in name2idx.items():
            conn_name = name  # profiles_raw keyed by idx
            if idx in profiles_raw:
                class_vecs[idx] = torch.tensor(profiles_raw[idx])
        data['ingredient'].x = torch.cat([one_hot, class_vecs], dim=1)
    else:
        data['ingredient'].x = one_hot

    data['compound'].x = torch.eye(n_cmp)

    if ic_edges:
        src = torch.tensor([e[0] for e in ic_edges])
        dst = torch.tensor([e[1] for e in ic_edges])
        data['ingredient', 'has', 'compound'].edge_index = torch.stack([src, dst])
        data['compound', 'rev_has', 'ingredient'].edge_index = torch.stack([dst, src])

    ii_src, ii_dst, ii_w = [], [], []
    for (n1, n2, combined, pmi, _shared) in pairs:
        if n1 not in name2idx or n2 not in name2idx:
            continue
        i1, i2 = name2idx[n1], name2idx[n2]
        if config == 'C' and phi is not None and profiles_raw is not None:
            p1 = profiles_raw.get(i1)
            p2 = profiles_raw.get(i2)
            c_score = float(p1 @ phi @ p2) if (p1 is not None and p2 is not None) else 0.0
            w = pmi * c_score
        else:
            w = combined
        if w > 0:
            ii_src += [i1, i2]; ii_dst += [i2, i1]; ii_w += [w, w]

    if ii_src:
        data['ingredient', 'pairs', 'ingredient'].edge_index = torch.tensor([ii_src, ii_dst])
        data['ingredient', 'pairs', 'ingredient'].edge_attr  = torch.tensor(ii_w, dtype=torch.float).unsqueeze(1)

    return data


def train_and_score(model_type, recipe_data, n_ing, emb_np=None, profiles=None, seed=42):
    random.seed(seed); np.random.seed(seed); torch.manual_seed(seed)

    random.shuffle(recipe_data)
    split = int(0.8 * len(recipe_data))
    train_data = recipe_data[:split]
    test_data  = recipe_data[split:]

    def make_features(samples):
        X, y = [], []
        for (ctx, _target, cls) in samples:
            if model_type == 'A':
                # sum of class vectors
                ctx_vecs = [profiles[i] for i in ctx if i in profiles]
                if not ctx_vecs:
                    continue
                feat = np.sum(ctx_vecs, axis=0)
            else:
                # mean of GNN embeddings
                ctx_embs = [emb_np[i] for i in ctx]
                if not ctx_embs:
                    continue
                feat = np.mean(ctx_embs, axis=0)
            X.append(feat)
            y.append(cls)
        return np.array(X, dtype=np.float32), np.array(y, dtype=np.int64)

    X_train, y_train = make_features(train_data)
    X_test,  y_test  = make_features(test_data)

    if len(X_train) == 0 or len(X_test) == 0:
        return dict(acc1=0.0, acc3=0.0)

    in_dim = X_train.shape[1]
    clf = nn.Sequential(nn.Linear(in_dim, 64), nn.ReLU(), nn.Linear(64, K))
    opt = torch.optim.Adam(clf.parameters(), lr=1e-3)

    Xt = torch.tensor(X_train); yt = torch.tensor(y_train)
    for _ep in range(200):
        clf.train(); opt.zero_grad()
        loss = F.cross_entropy(clf(Xt), yt)
        loss.backward(); opt.step()

    clf.train(False)
    with torch.no_grad():
        logits = clf(torch.tensor(X_test))
        top1 = logits.argmax(dim=1).numpy()
        top3 = logits.topk(3, dim=1).indices.numpy()

    acc1 = float((top1 == y_test).mean())
    acc3 = float(np.array([y_test[i] in top3[i] for i in range(len(y_test))]).mean())
    return dict(acc1=acc1, acc3=acc3)


def main():
    print("Loading ingredients and profiles...")
    name2idx, idx2name, n_ing = load_ingredients()
    profiles = load_profiles(name2idx)

    # DB for graph
    conn = sqlite3.connect(DB_PATH)
    cmp_rows = conn.execute("SELECT id FROM compound ORDER BY id").fetchall()
    cmp_id2idx = {r[0]: i for i, r in enumerate(cmp_rows)}
    n_cmp = len(cmp_rows)
    ic_edges_raw = conn.execute("""
        SELECT i.name, ic.compound_id FROM ingredient_compound ic
        JOIN ingredient i ON i.id = ic.ingredient_id
    """).fetchall()
    ic_edges = [(name2idx[r[0].lower()], cmp_id2idx[r[1]])
                for r in ic_edges_raw if r[0].lower() in name2idx and r[1] in cmp_id2idx]
    pairs_raw = conn.execute(
        "SELECT ing_a, ing_b, combined_score, pmi_score, shared_count FROM pair_score"
    ).fetchall()
    seen, pairs = set(), []
    for r in pairs_raw:
        key = (min(r[0],r[1]), max(r[0],r[1]))
        if key not in seen and r[0].lower() in name2idx and r[1].lower() in name2idx:
            seen.add(key)
            pairs.append((r[0].lower(), r[1].lower(), float(r[2] or 0), float(r[3] or 0), float(r[4] or 0)))
    conn.close()

    # class PMI matrix for config C
    sums = np.zeros((K,K)); cnts = np.zeros((K,K))
    for n1, n2, _, pmi, _ in pairs:
        i1, i2 = name2idx.get(n1), name2idx.get(n2)
        if i1 is None or i2 is None: continue
        p1, p2 = profiles.get(i1), profiles.get(i2)
        if p1 is None or p2 is None: continue
        c1, c2 = int(np.argmax(p1)), int(np.argmax(p2))
        sums[c1,c2] += pmi; sums[c2,c1] += pmi
        cnts[c1,c2] += 1;  cnts[c2,c1] += 1
    with np.errstate(invalid='ignore'):
        phi = np.where(cnts > 0, sums / cnts, 0.0).astype(np.float32)
    mx = phi.max()
    if mx > 0: phi /= mx

    print("Building recipe dataset...")
    random.seed(42)
    recipe_data = build_recipe_dataset(name2idx, profiles, min_matched=3)
    print(f"  Recipes usable: {len(recipe_data)}")

    results = {}

    for cfg in ['A', 'B', 'C']:
        seed_metrics = []
        for seed in SEEDS:
            print(f"  [{cfg}] seed={seed} ...", flush=True)

            if cfg == 'A':
                m = train_and_score('A', recipe_data, n_ing, profiles=profiles, seed=seed)
            else:
                # train GNN
                g_config = 'A' if cfg == 'B' else 'C'
                graph = load_graph_data(name2idx, n_ing, n_cmp, cmp_id2idx,
                                        ic_edges, pairs, phi=phi, profiles_raw=profiles, config=g_config)
                in_dim = n_ing if cfg == 'B' else n_ing + K
                gnn = build_gnn_model(in_dim, n_cmp)
                opt = torch.optim.Adam(gnn.parameters(), lr=1e-3, weight_decay=1e-5)
                # simple BPR pretrain (200 ep)
                pos_pairs = [(name2idx[r[0]], name2idx[r[1]]) for r in pairs if r[0] in name2idx and r[1] in name2idx]
                torch.manual_seed(seed)
                for _ep in range(200):
                    gnn.train(); opt.zero_grad()
                    emb = gnn(graph)
                    if not pos_pairs: break
                    chosen = [pos_pairs[random.randint(0, len(pos_pairs)-1)] for _ in range(512)]
                    pi = torch.tensor([c[0] for c in chosen])
                    pj = torch.tensor([c[1] for c in chosen])
                    nk = torch.randint(0, n_ing, (512,))
                    loss = -F.logsigmoid((emb[pi]*emb[pj]).sum(1) - (emb[pi]*emb[nk]).sum(1)).mean()
                    loss.backward(); opt.step()
                gnn.train(False)
                with torch.no_grad():
                    emb_np = gnn(graph).numpy()
                m = train_and_score('GNN', recipe_data, n_ing, emb_np=emb_np, profiles=profiles, seed=seed)

            seed_metrics.append(m)
            print(f"    Acc@1={m['acc1']:.4f}  Acc@3={m['acc3']:.4f}")
        results[cfg] = seed_metrics

    print("\n=== 19: Masked Flavor Class Prediction ===\n")
    print(f"{'Model':<6} {'Acc@1':>10} {'Acc@3':>10}")
    print("-" * 28)
    for cfg, sm in results.items():
        a1 = [m['acc1'] for m in sm]; a3 = [m['acc3'] for m in sm]
        print(f"{cfg:<6} {np.mean(a1):.4f}±{np.std(a1):.4f}  {np.mean(a3):.4f}±{np.std(a3):.4f}")

    # random baseline
    print(f"{'random':<6} {1/K:.4f}           {3/K:.4f}")

    with open("scripts/gnn/19_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\nSaved → scripts/gnn/19_results.json")


if __name__ == "__main__":
    main()
