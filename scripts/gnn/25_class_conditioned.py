"""
25_class_conditioned.py — Class-Conditioned Relation Decoder
TC contribution: s(i,j) = R[c(i),c(j)] * (z_i . z_j) + bias[c(i),c(j)]
R is 10x10 scalar matrix initialized from class-level PMI (Phi).

Proper eval: dedup (ing_a < ing_b) + 80/20 train/test split.
Target: test Spearman > 1L-HGN baseline (0.7174)
"""
...
def load_data():
    ...
    class_assign = np.zeros(n_ing, dtype=np.int64)
    for name, vec in raw_profiles.items():
        if name in name2id:
            idx = name2id[name] - 1
            total = max(float(vec.get("total", 1)), 1.0)
            vals = [float(vec.get(k, 0.0)) / total for k in CLASS_KEYS]
            class_assign[idx] = int(np.argmax(vals))

    # Class-level PMI matrix Phi
    phi = np.zeros((N_CLASS, N_CLASS), dtype=np.float32)
    counts = np.zeros((N_CLASS, N_CLASS), dtype=np.int32)
    for i, j, pmi in pairs:
        c1, c2 = class_assign[i], class_assign[j]
        phi[c1, c2] += pmi
        phi[c2, c1] += pmi
        counts[c1, c2] += 1
        counts[c2, c1] += 1
...
class ClassConditionedHGN(nn.Module):
    """s(i,j) = R[c(i),c(j)] * (z_i . z_j) + bias[c(i),c(j)]"""
    def __init__(self, n_ing, n_comp, hidden, n_class, phi_init):
        super().__init__()
        self.encoder = HGTEncoder(n_ing, n_comp, hidden)
        phi_t = torch.tensor(phi_init, dtype=torch.float)
        phi_min, phi_max = phi_t.min(), phi_t.max()
        R_init = (phi_t - phi_min) / (phi_max - phi_min + 1e-8) * 0.4 + 0.8
        self.R = nn.Parameter(R_init)
        self.bias = nn.Parameter(torch.zeros(n_class, n_class))

    def forward_score(self, hdata, class_t, i_idx, j_idx):
        z = self.encoder(hdata)
        dot = (z[i_idx] * z[j_idx]).sum(-1)
        ci, cj = class_t[i_idx], class_t[j_idx]
        return self.R[ci, cj] * dot + self.bias[ci, cj]
...
def run_model(model, hdata, class_t, train_pairs, test_pairs, seed, time_budget, device):
    ...
    while time.time() - t0 < time_budget:
        ...
        if class_t is not None:
            ps = model.forward_score(hdata, class_t, i_tr[pp], j_tr[pp])
            ns = model.forward_score(hdata, class_t, i_tr[np_], j_tr[np_])
        else:
    ...
    with torch.no_grad():
        if class_t is not None:
            tr_sc = model.forward_score(hdata, class_t, i_tr, j_tr).cpu().numpy()
            te_sc = model.forward_score(hdata, class_t, i_te, j_te).cpu().numpy()
        else:
...
def main():
    ...
    print(f"  N_CLASS={N_CLASS}, target: 1L-HGN test baseline={HGN_BASELINE}")

    hdata = build_pyg_data(data, device)
    class_t = torch.tensor(data["class_assign"], dtype=torch.long, device=device)

    MODELS = [
        ("1L-HGN",          False),
        ("ClassConditioned", True),
    ]

    results = {}
    for tag, use_class in MODELS:
        print(f"\n--- {tag} ---")
        tr_list, te_list = [], []
        for seed in SEEDS:
            ...
            torch.manual_seed(seed)
            if use_class:
                model = ClassConditionedHGN(
                    data["n_ing"], data["n_comp"], 64, N_CLASS, data["phi"]
                ).to(device)
                ct = class_t
            else:
            ...
            tr_sp, te_sp = run_model(model, hdata, ct, train_p, test_p, seed, TIME_BUDGET, device)
    ...
    print("\n" + "="*65)
    print("=== 25: Class-Conditioned Decoder (Proper Eval) ===")
    print("="*65)
    print(f"  1L-HGN proper eval baseline: {HGN_BASELINE}")
