"""
Train FlavorHGN on the expanded 1830-ingredient graph.

Same pipeline as 03/04 but scaled:
- SimGRACE pre-training (200 epochs)
- BPR fine-tune on PMI pairs (300 epochs)

Evaluation uses sampled ranking (N=200 anchors, batch scoring) to avoid OOM.
Cross-signal Spearman uses the original 151-ingredient compound data from DB.
"""
...
GRAPH_PATH = os.path.join(os.path.dirname(__file__), 'graph_1840.pt')
PRETRAIN_CKPT = os.path.join(os.path.dirname(__file__), 'pretrain_1830.pt')
FINETUNE_CKPT = os.path.join(os.path.dirname(__file__), 'finetune_1830.pt')
...
def ranking_metrics(model, data, test_pairs, all_pairs, n_ing, k=10, n_samples=200, device='cpu'):
    """Batched ranking to avoid OOM on large graph."""
    pos_by_anchor = {}
    ...
    with torch.no_grad():
        ...
        emb = x_dict['ingredient']  # [1830, 128]
        W = model.decoder.W

    hit, ndcg, rr = [], [], []
    sample = random.sample(test_pairs, min(n_samples, len(test_pairs)))
    for anchor, pos_idx, _ in sample:
...
def run(
    ...
    # Build training pairs from graph PMI edges (one direction only)
    print("Building training pairs from PMI edges...")
    ...
    print(f"  {len(all_pairs)} unique pairs")
    ...
    print(f"Pre-training done. Best loss: {best_pre_loss:.4f}")
    ...
    for ep in range(1, finetune_epochs+1):
        ...
        if ep % 100 == 0:
            metrics = ranking_metrics(model, data, test_pairs, all_pairs, n_ing, device=device)
            sp = compound_spearman(model, data, device=device)
    ...
    sp_final = compound_spearman(model, data, device=device)
    for k, v in final.items():
        print(f"  {k}: {v:.4f}")
    print(f"  Cross-signal Spearman: {sp_final:.4f}")
